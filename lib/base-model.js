...
BaseModel.db = {};
BaseModel.connectionName = {};


BaseModel.connect = function(configs, callback) {

  Async.forEachOfSeries(configs, function(connectionConfig, connectionName, callback2) {
    Mongodb.MongoClient.connect(connectionConfig.url, connectionConfig.options, (err, db) => {
      if (err) {
        console.log(err);
        return callback2(err);
      }
      console.log(connectionName);
      BaseModel.db[connectionName] = db;
      return callback2(null, db);
    });
...
BaseModel.disconnect = function () {
    ...
    BaseModel.db[this.connectionName].close();
};
...
BaseModel.createIndexes = function () {
    ...
    const collection = BaseModel.db[this.connectionName].collection(this._collection);
